...
class Strategy(Pipeline):
    def init_properties(self):
        ...
        if self.args.warmup:
            self.train_dataset = f'summ-sft_{self.proposal_model()}_aligner_warmup'
            ...
            self.train_dataset = self.args.train_dataset or f'summ-sft_{self.proposal_model()}_aligner_common'
            self.args.archive = f'_output/model/wu_aligner_summ-sft_{self.args.model}_b64_lr1e-06/LATEST'
            self.warmup_steps = self.calc_examples_per_epoch() // self.args.batch_size // 10
        # extra tokens for "correction" indicator
        extra_length = 10
        self.args.max_length = 1664 + extra_length # 640 + 512 + 512
        self.args.max_prompt_length = 1152 + extra_length # 640 + 512
        self.use_vllm = True
