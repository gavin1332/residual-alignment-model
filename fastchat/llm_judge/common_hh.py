...
def run_judge_single(question, answer, judge, ref_answer, turn):
    # summ_prompt = """Which of the following summaries does a better job of summarizing the mostimportant points in the given forum post, without including unimportant orirrelevant details? A good summary is both precise and concise.\nPost :\n{context}\nSummary A:\n{model_res}\nSummary B:\n{gold_answer}\nFIRST provide a one-sentence comparison of the two summaries,explaining whichyou prefer and why.SECOND,on a new line, state only "A" or "B" to indicate yourchoice.Your response should use the format :Comparison:<one-sentence comparison and expanation>Preferred:<"A"or "B">"""
    diag_prompt = """For the following query to a chatbot, which response is more helpful?Your evaluation should consider factors such as the helpfulness, relevance, accuracy, depth, creativity, and level of detail of their responses.  Be as objective as possible.\n Context:{context}\nResponse A:\n{model_res}\nResponse B:\n{gold_answer}\nFIRST provide a one-sentence comparison of the two responses and explain which you feel is more helpful. SECOND, on a new line, state only "A" or"B" to indicate which response is more helpful. Your response should usethe format :\nComparison:<one-sentence comparison andexplanation>More helpful:<"A" or "B">"""
    ...
    user_prompt = diag_prompt.format(
        context=question,
        model_res=answer,
    ...
    if judge.prompt_template["output_format"] == "[[rating]]":
        one_ab_pattern = re.compile("More helpful:.?[AB]")
        match = re.search(one_ab_pattern, judgment)
        # if not match:
        #     match = re.search(one_score_pattern_backup, judgment)
        #
        if match:
            rating =match.group()[-1]
            if rating=="A" and turn == 1:
                rating = 1
            elif rating == "B" and turn == 1:
                rating = 0
            elif rating == "A" and turn == 2:
                rating = 0
            elif rating == "B" and turn == 2:
                rating = 1
            else:
                rating = -1
    else:
...
def play_a_match_single(question,question_id, model, answer, judge, ref_answer, turn, output_file: str):
    ...
    if judge.prompt_template["type"] == "single":
        score, user_prompt, judgment = run_judge_single(
            question, answer, judge, ref_answer, turn=turn
        )
...
base_url="http://10.11.3.29:9208/api/v1/"
client = OpenAI(
        api_key='EMPTY',
        base_url=base_url
    )
...
#         base_url=base_url
#     )
# model="gpt-4o-mini"  #"gpt-4o-2024-08-06" #'gpt-4-turbo'
# client = OpenAI(
#            api_key="sk-2MgItnafpHljGHoDEe78F4De26B54179A6Bb71A68b7fF765",
...
def chat_completion_openai(model, conv, temperature, max_tokens, api_dict=None):
    ...
    #openai.api_base="https://api.bltcy.ai"
    #openai.api_key="sk-rd1Mog2Lc8BZTpjTCc59F21d90A444D7AcCb5aB4Ae986fF7"
