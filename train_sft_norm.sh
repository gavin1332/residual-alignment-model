...
#
#model=pythia28
#exp_name=pythia28b_sft_our-5e7-00001
#CUDA_VISIBLE_DEVICES=0,1,2,5 python -u train.py model=$model datasets=[alpaca_sample] loss=sft exp_name=$exp_name gradient_accumulation_steps=1 batch_size=16 eval_batch_size=5 trainer=FSDPTrainer sample_during_eval=false model.fsdp_policy_mp=bfloat16
model=pythia12b
exp_name=pythia12b_sft_our_summ
lr=1e-6
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 python -u train.py model=$model datasets=[summ] loss=sft_norm lr=$lr exp_name=$exp_name gradient_accumulation_steps=1 batch_size=12 eval_batch_size=6 trainer=FSDPTrainer sample_during_eval=false model.fsdp_policy_mp=bfloat16
